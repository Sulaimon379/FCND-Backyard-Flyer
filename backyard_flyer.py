...
class BackyardFlyer(Drone):
	...
	def local_position_callback(self):
		...
		if self.flight_state == States.WAYPOINT:
			...
			distance =  math.sqrt( pow(local_pos[0] - target_pos[0], 2) + pow(local_pos[1] - target_pos[1], 2) + pow(local_pos[2] + target_pos[2], 2))
			#if np.linalg.norm(target_pos[0:2] - local_pos[0:2]) < 1.0:
			if distance < 0.1:
	...
	def calculate_box(self):
		...
		"""
		waypoints = [[10.0, 0.0, 3.0, 0.0], [10.0, 10.0, 3.0, 0.0], [0.0, 10.0, 3.0, 0.0], [0.0, 0.0, 3.0, 0.0]]
		return waypoints
	...
	def waypoint_transition(self):
		...
		self.target_position = np.array(next_waypoint[0:3])
